...
GH_RUN_COUNT?=$(GITHUB_RUN_NUMBER)
CONFIG_PKG=github.com/itsivag/suprclaw/pkg/config
LDFLAGS=-X $(CONFIG_PKG).Version=$(VERSION) -X $(CONFIG_PKG).GitCommit=$(GIT_COMMIT) -X $(CONFIG_PKG).BuildTime=$(BUILD_TIME) -X $(CONFIG_PKG).GoVersion=$(GO_VERSION) -X $(CONFIG_PKG).GHRunCount=$(GH_RUN_COUNT) -s -w
...
SUPRCLAW_HOME?=$(HOME)/.suprclaw
WORKSPACE_DIR?=$(SUPRCLAW_HOME)/workspace
...
## build: Build the suprclaw binary for current platform
build: generate
...
## build-launcher: Build the suprclaw-launcher (web console) binary
build-launcher:
	@echo "Building suprclaw-launcher for $(PLATFORM)/$(ARCH)..."
	@mkdir -p $(BUILD_DIR)
	@if [ ! -f web/backend/dist/index.html ]; then \
		echo "Building frontend..."; \
		cd web/frontend && pnpm install && pnpm build:backend; \
	fi
	@$(WEB_GO) build $(GOFLAGS) -o $(BUILD_DIR)/suprclaw-launcher-$(PLATFORM)-$(ARCH) ./web/backend
	@ln -sf suprclaw-launcher-$(PLATFORM)-$(ARCH) $(BUILD_DIR)/suprclaw-launcher
	@echo "Build complete: $(BUILD_DIR)/suprclaw-launcher"
...
## build-all: Build suprclaw for all platforms
build-all: generate
	@echo "Building for multiple platforms..."
	@mkdir -p $(BUILD_DIR)
...
## install: Install suprclaw to system and copy builtin skills
install: build
...
## uninstall: Remove suprclaw from system
uninstall:
...
## uninstall-all: Remove suprclaw and all data
uninstall-all:
	@echo "Removing workspace and skills..."
	@rm -rf $(SUPRCLAW_HOME)
	@echo "Removed workspace: $(SUPRCLAW_HOME)"
...
docker-build:
	...
	docker compose -f docker/docker-compose.yml build suprclaw-agent suprclaw-gateway

## docker-build-full: Build Docker image with full MCP support (Node.js 24)
docker-build-full:
	@echo "Building full-featured Docker image (Node.js 24)..."
	docker compose -f docker/docker-compose.full.yml build suprclaw-agent suprclaw-gateway
...
## docker-run: Run suprclaw gateway in Docker (Alpine-based)
docker-run:
	docker compose -f docker/docker-compose.yml --profile gateway up

## docker-run-full: Run suprclaw gateway in Docker (full-featured)
docker-run-full:
	docker compose -f docker/docker-compose.full.yml --profile gateway up

## docker-run-agent: Run suprclaw agent in Docker (interactive, Alpine-based)
docker-run-agent:
	docker compose -f docker/docker-compose.yml run --rm suprclaw-agent

## docker-run-agent-full: Run suprclaw agent in Docker (interactive, full-featured)
docker-run-agent-full:
	docker compose -f docker/docker-compose.full.yml run --rm suprclaw-agent

## docker-clean: Clean Docker images and volumes
docker-clean:
	docker compose -f docker/docker-compose.yml down -v
	docker compose -f docker/docker-compose.full.yml down -v
	docker rmi suprclaw:latest suprclaw:full 2>/dev/null || true

## help: Show this help message
help:
	@echo "suprclaw Makefile"
	@echo ""
	@echo "Usage:"
	@echo "  make [target]"
	@echo ""
	...
	@echo "  WORKSPACE_DIR           # Workspace directory (default: ~/.suprclaw/workspace)"
