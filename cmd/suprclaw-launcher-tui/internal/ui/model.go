	...
	items = append(items,
		MenuItem{
			...
			Action: func() {
				...
				s.addModel(
					suprclawconfig.ModelConfig{ModelName: newName, Model: "openai/gpt-5.4"},
				)
				s.push(
...
func (s *appState) addModel(model suprclawconfig.ModelConfig) {
...
func refreshModelMenu(menu *Menu, currentModel string, models []suprclawconfig.ModelConfig) {
...
func isModelValid(model suprclawconfig.ModelConfig) bool {
...
func (s *appState) testModel(model *suprclawconfig.ModelConfig) {
	if model == nil {
		return
	}
