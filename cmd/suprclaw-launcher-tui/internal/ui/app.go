...
func Run() error {
	...
	if cfg == nil {
		cfg = suprclawconfig.DefaultConfig()
	}
...
func refreshMainMenu(menu *Menu, s *appState) {
	...
	items := []MenuItem{
		...
		{
			Label:       "Start Talk",
			Description: "Open suprclaw agent in terminal",
			Action: func() {
				s.requestStartTalk()
			},
			Disabled: !modelReady,
...
func (s *appState) startTalk() {
	...
	s.app.Suspend(func() {
		cmd := exec.Command("suprclaw", "agent")
		cmd.Stdin = os.Stdin
...
func (s *appState) startGateway() {
	...
	cmd := exec.Command("suprclaw", "gateway")
...
func (s *appState) hasEnabledChannel() bool {
	c := s.config.Channels
	return c.Telegram.Enabled || c.Discord.Enabled ||
		c.WhatsApp.Enabled || c.Slack.Enabled ||
		c.Matrix.Enabled || c.LINE.Enabled
}
