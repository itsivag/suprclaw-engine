...
import (
	...
	"github.com/itsivag/suprclaw/pkg/migrate"
)
...
func NewMigrateCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "migrate",
		Short: "Migrate from xxxclaw(openclaw, etc.) to suprclaw",
		Args:  cobra.NoArgs,
		Example: `  suprclaw migrate
  suprclaw migrate --from openclaw
  suprclaw migrate --dry-run
  suprclaw migrate --refresh
  suprclaw migrate --force`,
	...
	cmd.Flags().StringVar(&opts.TargetHome, "target-home", "",
		"Override target home directory (default: ~/.suprclaw)")
