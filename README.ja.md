...
<h1>SuprClaw: Go で書かれた超効率 AI アシスタント</h1>
...
> **SuprClaw** は [Sipeed](https://sipeed.com) が立ち上げた独立したオープンソースプロジェクトです。完全に **Go 言語**で一から書かれており、OpenClaw、NanoBot、その他のプロジェクトのフォークではありません。

🦐 SuprClaw は [NanoBot](https://github.com/HKUDS/nanobot) にインスパイアされた超軽量パーソナル AI アシスタントです。Go でゼロからリファクタリングされ、AI エージェント自身がアーキテクチャの移行とコード最適化を推進するセルフブートストラッピングプロセスで構築されました。
...
## 📢 ニュース
2026-02-09 🎉 SuprClaw リリース！$10 ハードウェアで 10MB 未満の RAM で動く AI エージェントを 1 日で構築。🦐 行くぜ、シャコ！
...
### 🐜 革新的な省フットプリントデプロイ
SuprClaw はほぼすべての Linux デバイスにデプロイできます！
...
[リリースページ](https://github.com/sipeed/suprclaw/releases) からお使いのプラットフォーム用のファームウェアをダウンロードしてください。
...
Docker Compose を使えば、ローカルにインストールせずに SuprClaw を実行できます。
...
> [!TIP]
> **Docker ユーザー**: デフォルトでは、Gateway は `127.0.0.1` でリッスンしており、ホストからアクセスできません。ヘルスチェックエンドポイントにアクセスしたり、ポートを公開したりする必要がある場合は、環境変数で `SUPRCLAW_GATEWAY_HOST=0.0.0.0` を設定するか、`config.json` を更新してください。
...
> [!TIP]
> `~/.suprclaw/config.json` に API キーを設定してください。API キーの取得先: [Volcengine (CodingPlan)](https://www.volcengine.com/activity/codingplan?utm_campaign=SuprClaw&utm_content=SuprClaw&utm_medium=devrel&utm_source=OWO&utm_term=SuprClaw) (LLM) · [OpenRouter](https://openrouter.ai/keys) (LLM) · [Zhipu](https://open.bigmodel.cn/usercenter/proj-mgmt/apikeys) (LLM)。Web 検索は **任意** です — 無料の [Tavily API](https://tavily.com) (月 1000 クエリ無料) または [Brave Search API](https://brave.com/search/api) (月 2000 クエリ無料)。
...
> `request_timeout` は任意の秒単位設定です。省略または `<= 0` の場合、SuprClaw はデフォルトのタイムアウト（120秒）を使用します。
...
Telegram、Discord、QQ、DingTalk、LINE、WeCom で SuprClaw と会話できます
...
> **Docker Compose**: Gateway HTTP サーバーは共有の `127.0.0.1:18790` で Webhook を提供します。ホストからアクセスするには `suprclaw-gateway` サービスに `ports: ["18790:18790"]` を追加してください。
...
SuprClaw は3種類の WeCom 統合をサポートしています：
...
設定ファイル: `~/.suprclaw/config.json`

### 環境変数

環境変数を使用してデフォルトのパスを上書きできます。これは、ポータブルインストール、コンテナ化されたデプロイメント、または suprclaw をシステムサービスとして実行する場合に便利です。これらの変数は独立しており、異なるパスを制御します。

| 変数              | 説明                                                                                                                             | デフォルトパス            |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `SUPRCLAW_CONFIG` | 設定ファイルへのパスを上書きします。これにより、suprclaw は他のすべての場所を無視して、指定された `config.json` をロードします。 | `~/.suprclaw/config.json` |
| `SUPRCLAW_HOME`   | suprclaw データのルートディレクトリを上書きします。これにより、`workspace` やその他のデータディレクトリのデフォルトの場所が変更されます。          | `~/.suprclaw`             |

**例：**

```bash
# 特定の設定ファイルを使用して suprclaw を実行する
# ワークスペースのパスはその設定ファイル内から読み込まれます
SUPRCLAW_CONFIG=/etc/suprclaw/production.json suprclaw gateway

# すべてのデータを /opt/suprclaw に保存して suprclaw を実行する
# 設定はデフォルトの ~/.suprclaw/config.json からロードされます
# ワークスペースは /opt/suprclaw/workspace に作成されます
...
SuprClaw は設定されたワークスペース（デフォルト: `~/.suprclaw/workspace`）にデータを保存します：
...
SuprClaw はデフォルトでサンドボックス環境で実行されます。エージェントは設定されたワークスペース内のファイルにのみアクセスし、コマンドを実行できます。
...
| オプション | デフォルト | 説明 |
|-----------|-----------|------|
| `workspace` | `~/.suprclaw/workspace` | エージェントの作業ディレクトリ |
...
SuprClaw は自動的に定期タスクを実行できます。ワークスペースに `HEARTBEAT.md` ファイルを作成します：
...
**環境変数:**
- `SUPRCLAW_HEARTBEAT_ENABLED=false` で無効化
- `SUPRCLAW_HEARTBEAT_INTERVAL=60` で間隔変更
...
| `volcengine`             | LLM(Volcengine 直接)                   | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=SuprClaw&utm_content=SuprClaw&utm_medium=devrel&utm_source=OWO&utm_term=SuprClaw)           |
...
> **新機能！** SuprClaw は現在 **モデル中心** の設定アプローチを採用しています。`ベンダー/モデル` 形式（例: `zhipu/glm-4.7`）を指定するだけで、新しいプロバイダーを追加できます—**コードの変更は一切不要！**
...
| **VolcEngine (Doubao)** | `volcengine/` | `https://ark.cn-beijing.volces.com/api/v3` | OpenAI | [キーを取得](https://www.volcengine.com/activity/codingplan?utm_campaign=SuprClaw&utm_content=SuprClaw&utm_medium=devrel&utm_source=OWO&utm_term=SuprClaw) |
...
```
> OAuth認証を設定するには、`suprclaw auth login --provider anthropic` を実行してください。
...
同じモデル名で複数のエンドポイントを設定すると、SuprClaw が自動的にラウンドロビンで分散します：
...
| コマンド | 説明 |
|---------|------|
| `suprclaw onboard` | 設定＆ワークスペースの初期化 |
| `suprclaw agent -m "..."` | エージェントとチャット |
| `suprclaw agent` | インタラクティブチャットモード |
| `suprclaw gateway` | ゲートウェイを起動 |
| `suprclaw status` | ステータスを表示 |
...
検索 API キーをまだ設定していない場合、これは正常です。SuprClaw は手動検索用の便利なリンクを提供します。
...
別のインスタンスが実行中の場合に発生します。`suprclaw gateway` が 1 つだけ実行されていることを確認してください。
