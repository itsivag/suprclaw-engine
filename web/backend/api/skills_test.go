...
func TestHandleListSkills(t *testing.T) {
	...
	oldBuiltin := os.Getenv("SUPRCLAW_BUILTIN_SKILLS")
	if err := os.Setenv("SUPRCLAW_BUILTIN_SKILLS", builtinRoot); err != nil {
		t.Fatalf("Setenv(SUPRCLAW_BUILTIN_SKILLS) error = %v", err)
	}
	defer func() {
		if oldBuiltin == "" {
			_ = os.Unsetenv("SUPRCLAW_BUILTIN_SKILLS")
		} else {
			_ = os.Setenv("SUPRCLAW_BUILTIN_SKILLS", oldBuiltin)
		}
	}()
