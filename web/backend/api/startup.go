...
const (
	autoStartEntryName = "SuprClawLauncher"
	launchAgentLabel   = "io.suprclaw.launcher"
)
...
func linuxAutoStartPath() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".config", "autostart", "suprclaw-web.desktop")
}
...
func setLinuxAutoStart(enabled bool, exePath string, args []string) error {
	...
	if enabled {
		...
		content := strings.Join([]string{
			...
			"Version=1.0",
			"Name=SuprClaw Web",
			"Comment=Start SuprClaw Web on login",
