...
// SUPRCLAW_SSH_KEY_PATH to its path for the duration of the test. This is required
// whenever a test exercises encryption/decryption via credential.Encrypt or SaveConfig.
func mustSetupSSHKey(t *testing.T) {
	t.Helper()
	keyPath := filepath.Join(t.TempDir(), "suprclaw_ed25519.key")
	if err := credential.GenerateSSHKey(keyPath); err != nil {
		t.Fatalf("mustSetupSSHKey: %v", err)
	}
	t.Setenv("SUPRCLAW_SSH_KEY_PATH", keyPath)
}
...
func TestDefaultConfig_WorkspacePath_Default(t *testing.T) {
	...
	cfg := DefaultConfig()
	want := filepath.Join(fakeHome, ".suprclaw", "workspace")
...
func TestDefaultConfig_WorkspacePath_WithSuprclawHome(t *testing.T) {
	t.Setenv("SUPRCLAW_HOME", "/custom/suprclaw/home")

	cfg := DefaultConfig()
	want := filepath.Join("/custom/suprclaw/home", "workspace")

	if cfg.Agents.Defaults.Workspace != want {
		t.Errorf("Workspace path with SUPRCLAW_HOME = %q, want %q", cfg.Agents.Defaults.Workspace, want)
	}
}
...
func TestLoadConfig_WarnsForPlaintextAPIKey(t *testing.T) {
	...
	t.Setenv("SUPRCLAW_SSH_KEY_PATH", "")
...
// unchanged when SUPRCLAW_KEY_PASSPHRASE is not set.
func TestLoadConfig_NoSealWithoutPassphrase(t *testing.T) {
	...
	t.Setenv("SUPRCLAW_KEY_PASSPHRASE", "")
	t.Setenv("SUPRCLAW_SSH_KEY_PATH", "")
...
// TestLoadConfig_MixedKeys_NoPassphrase verifies that when SUPRCLAW_KEY_PASSPHRASE
