...
func setupWorkspace(t *testing.T, files map[string]string) string {
	t.Helper()
	tmpDir, err := os.MkdirTemp("", "suprclaw-test-*")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSingleSystemMessage(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			msgs := cb.BuildMessages(tt.history, tt.summary, tt.message, nil, "chat1", "test", "chat1", "", "")
			...
			sys := msgs[0].Content
			if !strings.Contains(sys, "suprclaw") {
...
func TestBuildMessages_CurrentSenderDynamicContext(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			msgs := cb.BuildMessages(nil, "", "hello", nil, "chat1", "discord", "chat1", tt.senderID, tt.senderDisplayName)
			sys := msgs[0].Content
...
// (~/.suprclaw/skills) invalidates the cached system prompt.
func TestGlobalSkillFileContentChange(t *testing.T) {
	...
	globalSkillPath := filepath.Join(tmpHome, ".suprclaw", "skills", "global-skill", "SKILL.md")
	...
	for g := range goroutines {
		...
		go func(id int) {
			...
			for i := range iterations {
				...
				if !strings.Contains(result, "suprclaw") {
					errs <- "prompt missing identity"
					return
				}

				// Also exercise BuildMessages concurrently
				msgs := cb.BuildMessages(nil, "", "hello", nil, "chat", "test", "chat", "", "")
				if len(msgs) < 2 {
...
func BenchmarkBuildMessagesWithCache(b *testing.B) {
	tmpDir, _ := os.MkdirTemp("", "suprclaw-bench-*")
	defer os.RemoveAll(tmpDir)
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = cb.BuildMessages(history, "summary", "new message", nil, "test", "cli", "test", "", "")
	}
}
