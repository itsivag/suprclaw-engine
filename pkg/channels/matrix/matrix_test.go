...
func TestMatrixLocalpartMentionRegexp(t *testing.T) {
	re := localpartMentionRegexp("suprclaw")

	cases := []struct {
		text string
		want bool
	}{
		{text: "@suprclaw hello", want: true},
		{text: "hi @suprclaw:matrix.org", want: true},
		{
			text: "\u6b22\u8fce\u4e00\u4e0bsuprclaw\u5c0f\u9f99\u867e",
...
func TestStripUserMention(t *testing.T) {
	userID := id.UserID("@suprclaw:matrix.org")

	cases := []struct {
		in   string
		want string
	}{
		{in: "@suprclaw:matrix.org hello", want: "hello"},
		{in: "@suprclaw, hello", want: "hello"},
...
func TestIsBotMentioned(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			msg: event.MessageEventContent{
				...
				Mentions: &event.Mentions{
					UserIDs: []id.UserID{id.UserID("@suprclaw:matrix.org")},
				},
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body: "@suprclaw:matrix.org hello",
			},
			want: true,
		},
		{
		...
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/@suprclaw:matrix.org">SuprClaw</a> hello`,
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/%40suprclaw%3Amatrix.org">SuprClaw</a> hello`,
			},
			want: true,
		},
	}
...
func TestDownloadMedia_WritesResponseToTempFile(t *testing.T) {
	...
	client, err := mautrix.NewClient(server.URL, id.UserID("@suprclaw:matrix.test"), "")
	if err != nil {
