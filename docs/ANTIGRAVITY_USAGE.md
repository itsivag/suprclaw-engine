# Using Antigravity Provider in SuprClaw

This guide explains how to set up and use the **Antigravity** (Google Cloud Code Assist) provider in SuprClaw.
...
SuprClaw will extract the authorization code and complete the process automatically.
...
### Switch Models
You can change the default model in `~/.suprclaw/config.json` or override it via the CLI:
...
1.  **Environment Variables**:
    *   `SUPRCLAW_AGENTS_DEFAULTS_MODEL=gemini-flash`
...
*   **429 Rate Limit**: Antigravity has strict quotas. SuprClaw will display the "reset time" in the error message if you hit a limit.
*   **404 Not Found**: Ensure you are using a model ID from the `suprclaw auth models` list. Use the short ID (e.g., `gemini-3-flash`) not the full path.
