...
**Antigravity** (Google Cloud Code Assist) is a Google-backed AI model provider that offers access to models like Claude Opus 4.6 and Gemini through Google's Cloud infrastructure. This document provides a complete guide on how authentication works, how to fetch models, and how to implement a new provider in SuprClaw.
...
10. [Creating a New Provider in SuprClaw](#creating-a-new-provider-in-suprclaw)
...
const CLIENT_ID = decode(
  "REDACTED_GOOGLE_CLIENT_ID_BASE64="
);
const CLIENT_SECRET = decode("REDACTED_GOOGLE_CLIENT_SECRET_BASE64");
```
...
const antigravityPlugin = {
  ...
  register(api: SuprClawPluginApi) {
...
type ProviderAuthResult = {
  ...
  configPatch?: Partial<SuprClawConfig>;
  defaultModel?: string;
  notes?: string[];
};
```
...
- Go ≥ 1.21
- SuprClaw codebase (`pkg/providers/` and `pkg/auth/`)
...
Auth profiles are stored in `~/.suprclaw/auth.json`:
...
## Creating a New Provider in SuprClaw

SuprClaw providers are implemented as Go packages under `pkg/providers/`. To add a new provider:
...
If your provider requires OAuth or special authentication, add a case to `cmd/suprclaw/cmd_auth.go`:
...
export SUPRCLAW_AGENTS_DEFAULTS_MODEL=your-model

# Override provider settings
export SUPRCLAW_MODEL_LIST='[{"model_name":"your-model","model":"your-provider/model-name","api_key":"..."}]'
```
...
- **Source Files:**
  ...
  - `pkg/auth/store.go` - Auth credential storage (`~/.suprclaw/auth.json`)
...
### "Token expired"
- Refresh OAuth tokens: `suprclaw auth login --provider antigravity`
...
- Check auth profile storage: `~/.suprclaw/auth.json`
- Re-run `suprclaw auth login --provider antigravity`
