...
<h1>SuprClaw: Trợ lý AI Siêu Nhẹ viết bằng Go</h1>
...
> **SuprClaw** là dự án mã nguồn mở độc lập được khởi xướng bởi [Sipeed](https://sipeed.com). Được viết hoàn toàn bằng **Go** — không phải là bản fork của OpenClaw, NanoBot hay bất kỳ dự án nào khác.

🦐 **SuprClaw** là trợ lý AI cá nhân siêu nhẹ, lấy cảm hứng từ [NanoBot](https://github.com/HKUDS/nanobot), được viết lại hoàn toàn bằng **Go** thông qua quá trình "tự khởi tạo" (self-bootstrapping) — nơi chính AI Agent đã tự dẫn dắt toàn bộ quá trình chuyển đổi kiến trúc và tối ưu hóa mã nguồn.
...
>
> * **KHÔNG CÓ CRYPTO:** SuprClaw **KHÔNG** có bất kỳ token/coin chính thức nào. Mọi thông tin trên `pump.fun` hoặc các sàn giao dịch khác đều là **LỪA ĐẢO**.
> * **DOMAIN CHÍNH THỨC:** Website chính thức **DUY NHẤT** là **[suprclaw.io](https://suprclaw.io)**, website công ty là **[sipeed.com](https://sipeed.com)**.
> * **Cảnh báo:** Nhiều tên miền `.ai/.org/.com/.net/...` đã bị bên thứ ba đăng ký, không phải của chúng tôi.
> * **Cảnh báo:** SuprClaw đang trong giai đoạn phát triển sớm và có thể còn các vấn đề bảo mật mạng chưa được giải quyết. Không nên triển khai lên môi trường production trước phiên bản v1.0.
> * **Lưu ý:** SuprClaw gần đây đã merge nhiều PR, dẫn đến bộ nhớ sử dụng có thể lớn hơn (10–20MB) ở các phiên bản mới nhất. Chúng tôi sẽ ưu tiên tối ưu tài nguyên khi bộ tính năng đã ổn định.


## 📢 Tin tức

2026-02-16 🎉 SuprClaw đạt 12K stars chỉ trong một tuần! Cảm ơn tất cả mọi người! SuprClaw đang phát triển nhanh hơn chúng tôi tưởng tượng. Do số lượng PR tăng cao, chúng tôi cấp thiết cần maintainer từ cộng đồng. Các vai trò tình nguyện viên và roadmap đã được công bố [tại đây](docs/ROADMAP.md) — rất mong đón nhận sự tham gia của bạn!

2026-02-13 🎉 SuprClaw đạt 5000 stars trong 4 ngày! Cảm ơn cộng đồng! Chúng tôi đang hoàn thiện **Lộ trình dự án (Roadmap)** và thiết lập **Nhóm phát triển** để đẩy nhanh tốc độ phát triển SuprClaw.
...
2026-02-09 🎉 SuprClaw chính thức ra mắt! Được xây dựng trong 1 ngày để mang AI Agent đến phần cứng $10 với RAM <10MB. 🦐 SuprClaw, Lên Đường!
...
SuprClaw có thể triển khai trên hầu hết mọi thiết bị Linux!
...
Tải file binary cho nền tảng của bạn từ [trang Release](https://github.com/sipeed/suprclaw/releases).
...
Bạn cũng có thể chạy SuprClaw bằng Docker Compose mà không cần cài đặt gì trên máy.
...
> [!TIP]
> **Người dùng Docker**: Theo mặc định, Gateway lắng nghe trên `127.0.0.1`, không thể truy cập từ máy chủ. Nếu bạn cần truy cập các endpoint kiểm tra sức khỏe hoặc mở cổng, hãy đặt `SUPRCLAW_GATEWAY_HOST=0.0.0.0` trong môi trường của bạn hoặc cập nhật `config.json`.
...
```bash
# Đặt câu hỏi
docker compose -f docker/docker-compose.yml run --rm suprclaw-agent -m "2+2 bằng mấy?"
...
> [!TIP]
> Thiết lập API key trong `~/.suprclaw/config.json`. Lấy API key: [Volcengine (CodingPlan)](https://www.volcengine.com/activity/codingplan?utm_campaign=SuprClaw&utm_content=SuprClaw&utm_medium=devrel&utm_source=OWO&utm_term=SuprClaw) (LLM) · [OpenRouter](https://openrouter.ai/keys) (LLM) · [Zhipu](https://open.bigmodel.cn/usercenter/proj-mgmt/apikeys) (LLM). Tìm kiếm web là **tùy chọn** — lấy [Tavily API](https://tavily.com) miễn phí (1000 truy vấn/tháng) hoặc [Brave Search API](https://brave.com/search/api) (2000 truy vấn/tháng).
...
**2. Cấu hình** (`~/.suprclaw/config.json`)
...
> `request_timeout` là tùy chọn và dùng đơn vị giây. Nếu bỏ qua hoặc đặt `<= 0`, SuprClaw sẽ dùng timeout mặc định (120s).
...
```bash
suprclaw agent -m "Xin chào, bạn là ai?"
```
...
Trò chuyện với SuprClaw qua Telegram, Discord, DingTalk, LINE hoặc WeCom.
...
SuprClaw hỗ trợ ba loại tích hợp WeCom:
...
Kết nối SuprClaw với Mạng xã hội Agent chỉ bằng cách gửi một tin nhắn qua CLI hoặc bất kỳ ứng dụng Chat nào đã tích hợp.

**Đọc `https://clawdchat.ai/skill.md` và làm theo hướng dẫn để tham gia [ClawdChat.ai](https://clawdchat.ai)**

## ⚙️ Cấu hình chi tiết

File cấu hình: `~/.suprclaw/config.json`

### Biến môi trường

Bạn có thể ghi đè các đường dẫn mặc định bằng cách sử dụng các biến môi trường. Điều này hữu ích cho việc cài đặt di động, triển khai container hóa hoặc chạy suprclaw như một dịch vụ hệ thống. Các biến này độc lập và kiểm soát các đường dẫn khác nhau.

| Biến              | Mô tả                                                                                                                             | Đường dẫn mặc định        |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `SUPRCLAW_CONFIG` | Ghi đè đường dẫn đến file cấu hình. Điều này trực tiếp yêu cầu suprclaw tải file `config.json` nào, bỏ qua tất cả các vị trí khác. | `~/.suprclaw/config.json` |
| `SUPRCLAW_HOME`   | Ghi đè thư mục gốc cho dữ liệu suprclaw. Điều này thay đổi vị trí mặc định của `workspace` và các thư mục dữ liệu khác.          | `~/.suprclaw`             |

**Ví dụ:**

```bash
# Chạy suprclaw bằng một file cấu hình cụ thể
# Đường dẫn workspace sẽ được đọc từ trong file cấu hình đó
SUPRCLAW_CONFIG=/etc/suprclaw/production.json suprclaw gateway

# Chạy suprclaw với tất cả dữ liệu được lưu trữ trong /opt/suprclaw
# Cấu hình sẽ được tải từ ~/.suprclaw/config.json mặc định
# Workspace sẽ được tạo tại /opt/suprclaw/workspace
...
SuprClaw lưu trữ dữ liệu trong workspace đã cấu hình (mặc định: `~/.suprclaw/workspace`):
...
SuprClaw chạy trong môi trường sandbox theo mặc định. Agent chỉ có thể truy cập file và thực thi lệnh trong phạm vi workspace.
...
|----------|---------|-------|
| `workspace` | `~/.suprclaw/workspace` | Thư mục làm việc của agent |
...
SuprClaw có thể tự động thực hiện các tác vụ định kỳ. Tạo file `HEARTBEAT.md` trong workspace:
...
* `SUPRCLAW_HEARTBEAT_ENABLED=false` để tắt
* `SUPRCLAW_HEARTBEAT_INTERVAL=60` để thay đổi khoảng thời gian
...
| `volcengine`             | LLM(Volcengine trực tiếp)                   | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=SuprClaw&utm_content=SuprClaw&utm_medium=devrel&utm_source=OWO&utm_term=SuprClaw)           |
...
> **Tính năng mới!** SuprClaw hiện sử dụng phương pháp cấu hình **đặt mô hình vào trung tâm**. Chỉ cần chỉ định dạng `nhà cung cấp/mô hình` (ví dụ: `zhipu/glm-4.7`) để thêm nhà cung cấp mới—**không cần thay đổi mã!**
...
| **VolcEngine (Doubao)** | `volcengine/` | `https://ark.cn-beijing.volces.com/api/v3` | OpenAI | [Lấy Khóa](https://www.volcengine.com/activity/codingplan?utm_campaign=SuprClaw&utm_content=SuprClaw&utm_medium=devrel&utm_source=OWO&utm_term=SuprClaw) |
...
```
> Chạy `suprclaw auth login --provider anthropic` để thiết lập thông tin xác thực OAuth.
...
Định cấu hình nhiều endpoint cho cùng một tên mô hình—SuprClaw sẽ tự động phân phối round-robin giữa chúng:
...
| Lệnh | Mô tả |
| --- | --- |
| `suprclaw onboard` | Khởi tạo cấu hình & workspace |
| `suprclaw agent -m "..."` | Trò chuyện với agent |
| `suprclaw agent` | Chế độ chat tương tác |
| `suprclaw gateway` | Khởi động gateway (cho bot chat) |
| `suprclaw status` | Hiển thị trạng thái |
| `suprclaw cron list` | Liệt kê tất cả tác vụ định kỳ |
| `suprclaw cron add ...` | Thêm tác vụ định kỳ |

### Tác vụ định kỳ / Nhắc nhở

SuprClaw hỗ trợ nhắc nhở theo lịch và tác vụ lặp lại thông qua công cụ `cron`:
...
Các tác vụ được lưu trong `~/.suprclaw/workspace/cron/` và được xử lý tự động.
...
Điều này là bình thường nếu bạn chưa cấu hình API key cho tìm kiếm. SuprClaw sẽ cung cấp các liên kết hữu ích để tìm kiếm thủ công.
...
Thêm key vào `~/.suprclaw/config.json` nếu dùng Brave:
...
Điều này xảy ra khi có một instance bot khác đang chạy. Đảm bảo chỉ có một tiến trình `suprclaw gateway` chạy tại một thời điểm.
